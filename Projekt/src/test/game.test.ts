...
describe('POST /games/', () => {
  it('POST working as intended', () => {
    return chai.request(app).post('/games/')
    .send(spiel_neu)
    .then(res => {
...
describe('DELETE /games/:id', () => {
  it('DELETE working as intended', async () => {
   let game = await chai.request(app).post('/games/')
    .send(spiel_neu)
    .then(res => {
      return res.body.createdGame._id;
    });
    return chai.request(app).del('/games/' + game)
    .then(res => {
      expect(res.status).to.equal(200);
      expect(res).to.be.json;
...
describe('GET /games/:id', () => {
  it('findbyid working as intended', async () => {
   let game = await chai.request(app).get('/games/')
    .then(res => {
      return res.body.games[0]._id;
    });
    return chai.request(app).get('/games/' + game)
    .then(res => {
      expect(res.status).to.equal(200);
      expect(res).to.be.json;
