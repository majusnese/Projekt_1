...
export class SellerRouter {
    ...
    public async create(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            err => {
                logger.error(`Create seller failed due to a wrong ID ${stringify(err)}`);
                res.status(422).json({
        ...
        let check = await Game.findById(id)
            ...
            .catch(error => {
                logger.error(`Update failed while trying to find correlated Game: ${stringify(error)}`);
            });
        ...
        if (check) {
            ...
            if (isSeller(seller)) {
                seller
                    ...
                    .catch(err => {
                        logger.error(`Post seller Error while trying to update: ${stringify(err)}`);
                    });
            } else {
    ...
    public async findbyid(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            err => {
                logger.error(`Findbyid seller Error because an invalid id was passed: ${stringify(err)}`);
                res.status(422).json({
                    message: 'Please pass a valid ID',
                });
            };
        ...
        Seller.findById(id)
            ...
            .catch(err => {
                logger.error(`Findbyid seller Error while executing operation: ${stringify(err)}`);
            });
    }
    ...
    public async patch(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            err => {
                logger.error(`Update seller error because an invalid id was passed: ${stringify(err)}`);
                res.status(422).json({
                    message: 'Please pass a valid ID',
                });
            };
        ...
        let sellerInstance = await Seller.findById(id)
            ...
            .catch(error => {
                logger.error(`Update seller error while trying to find the seller: ${stringify(error)}`);
            });

        if (sellerInstance) {
            const updateOperations = {};
            for (const ops of req.body) {
                if (!isPropNameSeller(ops.propName) || !isValidValueSeller(ops.propName, ops.value)) {
                    logger.error(`Update seller failed because unprocessable arguments were passed!`);
                    res.status(422).json({
            ...
            Seller.update({ _id: id }, { $set: updateOperations })
                ...
                .catch(err => {
                    logger.error(`Update seller Error while trying to update: ${stringify(err)}`);
                });
        } else {
    ...
    public async del(req: Request, res: Response, next: NextFunction) {
        ...
        Seller.findById(id)
            ...
            .catch(err => {
                logger.error(`Del seller Error while trying to find the seller: ${stringify(err)}`);
            });
    }
